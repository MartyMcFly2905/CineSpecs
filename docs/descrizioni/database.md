...
* Un **UTENTE** può votare un tag una sola volta
...
* Non è prevista una gestione avanzata di ban o soft delete utente
...
* L'autore del film è tracciato nel database
...
* L'autore del frame è utile per dashboard e moderazione
...
* Il tag ha già autore esplicito tramite `id_utente`
...
* `id_tag` (FK → TAGS)
* `id_utente` (FK → UTENTI)
* `upvote` (`1` se il voto e positivo, altrimenti `0`)
* `downvote` (`1` se il voto e negativo, altrimenti `0`)
* `creato_il` (timestamp)
...
* ON DELETE CASCADE
* CHECK che impedisce di avere insieme upvote e downvote attivi
* UNIQUE `(id_tag, id_utente)`

### Note

* Ogni utente può votare una sola volta ogni tag
* I conteggi si ottengono con `SUM(upvote)` e `SUM(downvote)`
* La tabella basta per viewer, dashboard personale e moderazione admin
...
* la dashboard contributor è il punto principale di inserimento dati
* film, frame e hardware salvano sempre l'autore da sessione

### Voti sui TAG

* Solo utenti autenticati
* ogni utente ha un solo voto per tag
* voto ammesso: upvote o downvote
* il cambio voto aggiorna il record esistente

### Moderazione admin

* l'admin può rimuovere tag errati o malevoli
* il punteggio aiuta la moderazione ma non la sostituisce
* l'admin può intervenire anche su contenuti collegati se necessario
* l'admin deve vedere autore e data di creazione dei contenuti
...
* Uso di `id_utente_creatore` su film, frame e hardware → tracciabilità contributor semplice
* Uso di `creato_il` sui contenuti → utile per dashboard e log personale
* Uso di `RESTRICT` sulle foreign key autore → evita perdite accidentali di contenuti
...
* Nessun badge o reputazione avanzata
* La rimozione utenti richiederebbe una politica dedicata e non è prioritaria
